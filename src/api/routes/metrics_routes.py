...
async def get_metrics(
    ...
    fertilizer_costs = metric_service.calculate_fertilizer_cost(
        fertilizer_usage, FertilizerTypes.SOLID
    )
    ...
    potential_yield = await metric_service.calculate_yield(
        current_field=field, future_crop=next_crop
    )
    potential_profit = await metric_service.estimate_profit(
        current_field=field, estimated_yield=potential_yield, future_crop=next_crop
    )
    ...
    return MetricsResponseModel(
        ...
        estimated_yield=PotentialYieldModel(
            potential_yield=potential_yield, potential_profit=potential_profit
        ),
        estimated_fertilizer_usage=FertilizerUsageModel(
            fertilizer_usage=fertilizer_usage, fertilize_costs=fertilizer_costs
        ),
    )
